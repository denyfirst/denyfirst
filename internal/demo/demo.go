// Package demo holds the list of hosts the public deployment may connect to.
//
// It is not part of any check. What this deployment may reach is a property
// of the deployment, and every check asks the same question of it: the TLS
// scanner did until 2026-09-05, the web check does now, and a mail check
// will. Keeping the list inside the TLS scanner meant each of them importing
// the TLS scanner to find out, which is the shape that gets worse with every
// check added.
//
...
// ErrNotATarget is returned by a demonstration build for any host outside
// its list. It names no host back.
var ErrNotATarget = errors.New("this deployment demonstrates the tool on hosts it owns; run it yourself to scan anything else")

// isTarget reports whether a host is one the demonstration list covers.
//
...
func isTarget(host string) bool {
...
// Refusal reports whether a demonstration build refuses this host, so the
...
func Refusal(host string) bool {
	return Enabled && !isTarget(host)
}

// Targets returns the hosts a demonstration build will scan.
//
...
// Host is one entry on the demonstration menu.
//
...
// Hosts returns the menu a demonstration build offers.
//
// Empty in the ordinary build, where the person running the tool chooses.
func Hosts() []Host {
	out := make([]Host, len(demoHosts))
	copy(out, demoHosts)
	return out
}
