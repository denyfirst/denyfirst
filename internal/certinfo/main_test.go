...
this package is back to measuring untrusted chains and passing anyway.`, err)
	...
	if n := len(pool.Subjects()); n != 1 { //nolint:staticcheck // Subjects is deprecated for system pools; here the pool is ours
		t.Errorf("the test store holds %d certificates, not just the test root", n)
	}
}
